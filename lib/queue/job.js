...
/**
 * Set the job interval for repetition
 *
 * @param interval
 * @return {Job|Number}
 * @api public
...
/**
 * Next interval for the job if there is an interval set for it
 *
 * @api public
 */

Job.prototype.next = function(fn) {
    var self = this;
    if(self._interval) {
        var duplicate = new Job(this.type, this.data);
        duplicate._interval = this._interval;
        duplicate._priority = this._priority;
        duplicate.delay(moment().add(this._interval).fromNow());
        duplicate.save(fn);
    }
};
...
Job.prototype.update = function(fn){
  ...
  // interval
  if (this._interval) this.set('interval', this._interval);
